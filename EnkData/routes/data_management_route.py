...
def clean_dataset(dataset_id):
    ...
    if request.method == 'POST':
        ...
        #storing cleaned data in a session variable
        session['cleaned_dataset'] = cleaned_data
        #redirect to post cleaning
        return redirect(url_for('data_management.post_cleaning_options'))
...
# Route to display post-cleaning options
@data_management_bp.route('/post-cleaning-options/<int:dataset_id>', methods=['GET', 'POST'])
@login_required
def post_cleaning_options(dataset_id):
    # Retrieve the dataset by ID
    dataset = Dataset.query.get_or_404(dataset_id)
    ...
    if request.method == 'POST':
        ...
        if option == 'download':
            # Serve the cleaned dataset as a downloadable CSV file
            ...
            # Redirect to the data visualizations page
            return redirect(url_for('data_visualization.visualizations', dataset_id=dataset_id))

    return render_template('post_cleaning_options.html', dataset_available=True, dataset=dataset)
